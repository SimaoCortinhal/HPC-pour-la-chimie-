! genetic_docking.f90
! Fortran 90 implementation complète d’un algorithme génétique de docking rigide
...
contains
  ...
  integer function evaluate_hbonds(lig, nlig, site, nsite, lb, nlb, sb, nsb)
    ...
    real(kind=dp) :: d, angle, dx1, dy1, dz1, dx2, dy2, dz2, norm1, norm2, dot
...
program genetic_docking
  ...
  if (.not.(len_trim(out_dir)>0)) out_dir = 'results'
  ...
  do run = 1, runs
    ...
    do gen = 1, max_gen
      ...
      ! collecte des statistiques tous les 10 itérations
      ...
      ! recombinaison et mutation (mutation après crossover)
      do i = 1, pop_size, 2
    ...
    ! écriture des fichiers MOL2 de la population finale
    do i = 1, pop_size
      write(run_str,'(I2.2)') run
    ...
    ! écriture du CSV des statistiques
