! complete_mol2_omp.f90
program complete_mol2_omp
  use omp_lib
  implicit none
  ...
  integer :: pourcentage_delta, i, j, k
  real(kind=dp) :: delta_frac, distance_ij
  character(len=2) :: element_i, element_j
  real :: temps_debut, temps_fin


  ! Variables pour la lecture complète du fichier MOL2
  ...
  if (len_trim(fichier_mol2) == 0 .or. len_trim(fichier_covalence) == 0) then
    print *, "Usage: complete_mol2_omp <fichier.mol2> <fichier_rayons.txt>"
    stop
  end if
  ...
  do i = 1, nb_covalence
    write(*,'(A3,2X,F6.3,2X,F6.3,2X,F6.3)') trim(covalences(i)%element), &
         covalences(i)%rayon_simple, covalences(i)%rayon_double, covalences(i)%rayon_triple
  end do
  print *, "-----------"
  ...
  do pourcentage_delta = 10, 35, 5
    ...
    !$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(i,j,distance_ij,element_i,element_j) SCHEDULE(dynamic)
    do i = 1, nb_atomes - 1
      do j = i + 1, nb_atomes
        call calculate_distance(atomes, i, j, distance_ij)
        element_i = trim(atomes(i)%elem)
        element_j = trim(atomes(j)%elem)

        if ( try_bond_type_omp('triple', element_i, element_j, distance_ij, delta_frac, i, j, &
               covalences, nb_covalence, nombre_liaisons, liaisons) ) cycle
        if ( try_bond_type_omp('double', element_i, element_j, distance_ij, delta_frac, i, j, &
               covalences, nb_covalence, nombre_liaisons, liaisons) ) cycle
        if ( try_bond_type_omp('single', element_i, element_j, distance_ij, delta_frac, i, j, &
               covalences, nb_covalence, nombre_liaisons, liaisons) ) cycle
      end do
    end do
    !$OMP END PARALLEL DO
  ...
  do k = 1, nombre_liaisons
    ! Format standard pour MOL2 : bond_id, atome origine, atome destination, type de liaison
    write(10, '(I4,2X,I4,2X,I4,2X,A10)') k, liaisons(k)%a1, liaisons(k)%a2, trim(liaisons(k)%bond_type)
  end do
...
contains
  ...
  subroutine check_elements(atomes, nb_atomes, covalences, nb_covalence)
    type(atom_info), intent(in) :: atomes(:)
    integer, intent(in) :: nb_atomes, nb_covalence
    ...
    do i = 1, nb_atomes
      trouve = .false.
      do j = 1, nb_covalence
        if (trim(atomes(i)%elem) == trim(covalences(j)%element)) then
          trouve = .true.
          exit
        end if
      end do
      if (.not. trouve) then
        print *, "Element inconnu dans le fichier de covalence :", trim(atomes(i)%elem)
        stop
      end if
    end do
  ...
  logical function try_bond_type_omp(type_liaison, e1, e2, distance_ij, delta_frac, i, j, covalences, nb_covalence, nombre_liaisons, liaisons)
    character(len=*), intent(in) :: type_liaison, e1, e2
    real(kind=dp), intent(in) :: distance_ij, delta_frac
    integer, intent(in) :: i, j, nb_covalence
    type(covalence_data), intent(in) :: covalences(:)
    integer, intent(inout) :: nombre_liaisons
    type(bond), intent(inout) :: liaisons(:)
    real(kind=dp) :: r1, r2, somme_r, lower, upper
    r1 = get_radius(e1, type_liaison, covalences, nb_covalence)
    r2 = get_radius(e2, type_liaison, covalences, nb_covalence)
    somme_r = r1 + r2
    if (somme_r <= 0.0_dp) then
      try_bond_type_omp = .false.
      return
    end if
    lower = somme_r * (1.0_dp - delta_frac)
    upper = somme_r * (1.0_dp + delta_frac)
    if (distance_ij >= lower .and. distance_ij <= upper) then
      !$OMP CRITICAL(bond_update)
      nombre_liaisons = nombre_liaisons + 1
      liaisons(nombre_liaisons)%a1 = i
      liaisons(nombre_liaisons)%a2 = j
      select case(trim(type_liaison))
        case('single')
      ...
      !$OMP END CRITICAL(bond_update)
      try_bond_type_omp = .true.
    else
      try_bond_type_omp = .false.
    end if
  end function try_bond_type_omp

  real(kind=dp) function get_radius(e, type_liaison, covalences, nb_covalence)
    character(len=*), intent(in) :: e, type_liaison
    type(covalence_data), intent(in) :: covalences(:)
  ...
  subroutine uppercase(chaine)  
    character(len=*), intent(inout) :: chaine
    integer :: i, c
    do i = 1, len(chaine)
      c = ichar(chaine(i:i))
      if (c >= ichar('a') .and. c <= ichar('z')) then
        chaine(i:i) = char(c - 32)
      end if
    end do
  end subroutine uppercase

end program complete_mol2_omp
